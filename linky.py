...
def renderEnergy():
    ...
    # clear left side
    rect = [(textAnchorX, textAnchorY), (320, size[0] / 2)]
    lcd.fill(BLACK, rect)
    # render kWh string
    ...
    textrect.topleft = (195, textAnchorY)
    lcd.blit(text, textrect)
    lcd.blit(iconAlarmClock.bitmap, (165, textAnchorY + 5))

    # Render RSSI
    index = int(rssi) // 2 - 1  # get the right icon index
    lcd.blit(iconSignal[index].bitmap, (290, textAnchorY + 8))

    # Render Energy
    textAnchorY = + size[1] + 15
    size = fontTempHuge.size(energy)
    rect = [(textAnchorX, textAnchorY), (320, size[1])]
    ...
    lcd.blit(WattBlack, (284, textAnchorY + 6))
    pygame.display.update()
    ...
    lcd.fill(BLACK, rect)
    lcd.blit(text, textrect)
    lcd.blit(Watt, (284, textAnchorY + 6))
    pygame.display.update()
...
class RenderTimeThread(threading.Thread):
    ...
    def run(self):
        ...
        rect = [(textAnchorX, textAnchorY), (320, textAnchorY)]
...
iconAlarmClock = Icon(path, '/icons/time-5-24')
...
fontMono24 = pygame.font.Font(fontpath, 24)
fontTemperature = pygame.font.Font(fontpath, 40)
fontTime = pygame.font.Font(fontpath, 66)
fontTitle = pygame.font.Font(zfontpath, 48)
fontTempHuge = pygame.font.Font(zfontpath, 100)
...
marquee = Marquee(fontMono24, GREY, speed=1, ry=140)
        ...
        if animation:
            ...
            if angle == 0:
                rotated_image, rect = blitRotate(
                    lcd, image, (165 + w / 2, h / 2 + 5), (w / 2, h / 2), angle)
